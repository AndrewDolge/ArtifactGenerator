...
public class JsonArtifactComponentFactory implements IArtifactComponentFactory {
    ...
    public JsonArtifactComponentFactory(InputStream ... ins){
        ...
        for(InputStream in: ins){
            try (JsonReader reader = new JsonReader(new InputStreamReader(in))) {
                ...
                Type componentList = new TypeToken<LinkedList<SerializedArtifactComponent>>(){}.getType();
    ...
    private void processComponent(SerializedArtifactComponent ...components){
        ...
        for(SerializedArtifactComponent component: components){
            ...
            if(component != null){
                // if the component has a given category and independent data, add independent data to the descriptor
                if(component.hasIndependentData()){
                    builder
                    .withIndependentData(component.getData().toArray(new String[component.getData().size()]))
                ...
                if(component.hasDependentData()){
                    builder
                    .withDependentData(component.getDependentCategory(), component.getDependentData())
                    .withCategory(component.getCategory());
                }
                if(component.hasSelectionStrategy()){
                    builder
                    .withSelectionStrategy(component.getStrategyFromSelector());
                }
                ...
                if(component.isExclusive()){
                    filters.add(
                        new ConditionalDescriptorFilter(
                            FilterConditions.isCategoryPresent(component.getCategory()),
